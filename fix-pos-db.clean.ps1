function SafeWrite([string]$path,[string]$content){
  ...
  $tmp = Join-Path $dir ("_" + (Split-Path $path -Leaf) + ".tmp")
  Set-Content -Encoding UTF8 -Path $tmp -Value $content
...
SafeWrite ".\Models\CashierInvoice.cs" @'
...
SafeWrite ".\Models\CashierInvoiceItem.cs" @'
...
SafeWrite ".\Data\AppDbContext.cs" @'
...
# 4) (DISABLED) Restore fix skipped to avoid PowerShell quoting issues.
