...
Copy-Item $progPath "$progPath.bak_before_step3" -Force

$prog = Get-Content $progPath -Raw -Encoding UTF8

# احذف كل تعريفات Backup classes من Program.cs (من أول BackupSettings إلى نهاية الملف)
# ملاحظة: بدون \R وبدون \z
$prog = [regex]::Replace($prog, '(?s)\s*public\s+sealed\s+class\s+BackupSettings\b.*$', "`r`n")

if($prog -notmatch 'app\.Run\(\)\s*;'){
  throw "ما لقيت app.Run(); داخل Program.cs"
}

Set-Content -Encoding UTF8 -Path $progPath -Value $prog

# أنشئ ملف BackupFeature.cs للكلاسات
...
Write-Host "✅ OK: Program.cs cleaned + BackupFeature.cs created"
